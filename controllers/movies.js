...
const getMovies = require('../public/getMovies');
const searchMovies = require('../public/searchMovies');
...
router.post('/:profileId/movies/watchlist', authCheck.checkAuthenticated, async (req, res) => {
	try {
		if (!(await profileBelongsToUser(req)))
			throw new Error('This Profile does not belong to current User');
		...
		if (watchedMovie.length > 0) {
			await Movie.findOneAndUpdate(
				...
				{ watched: false }
			); //add movie to watchlist, change watched to false
